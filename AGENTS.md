...
When frontend files are edited, run the full frontend test suite with `npm run test` before finalizing.
If `npm run lint-fix` changes files, review the diff and rerun `npm run test`.
